...
class ProxyRotator():
    ...
    """
    def __init__(self, proxies:List[str]=None, get_proxies_retries:int=5, proxies_source:str='https://free-proxy-list.net/', default_proxy_protocol:str='http'):
        ...
        self.proxies = [default_proxy_protocol + '://' + proxy if not '://' in proxy else proxy for proxy in self.proxies]
        self.queue = []
        ...
        for i in range(retries):
            ...
            try:
                ...
                if proxy_response.status_code == 200:
                    ...
                    print(f"Failed to get proxies list from {source}. Status code: {proxy_response.status_code}")
                    continue
...
class GitHub_Crawler():
    ...
    def make_request(self, url:str, headers:dict = {}, proxy:str = None, timeout:int=None) -> str:
        ...
        try:
            response = requests.get(
                url, headers=headers, proxies={'http': proxy, 'https': proxy}, timeout=timeout
                ) if proxy else requests.get(
    ...
    def search(self, keywords:str|List[str], type:GitHub_Search_Type, proxies:List[str]=None, headers:dict = None, timeout:int = 30, retries:int = 5) -> List[dict]:
        """
